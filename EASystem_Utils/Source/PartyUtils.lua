...
--  d("set Assistant")
  local party, index
  party, index = PartyUtils.IsPlayerInWarband(SystemData.UserInput.selectedGroupMember)
  d(party.."   :   "..index)
end
...
----------------------------------------------------------------------------------------------------
function PartyUtils.SetMasterLooter()
    d("broadcast master loot")
    d(SystemData.Events.GROUP_SET_MASTER_LOOT_ON)
    d(SystemData.UserInput.selectedGroupMember)
